...
CREATE PROCEDURE [dbo].[sp_CpapCoachMain] 
...
BEGIN
		...
		--Note Bill ask that we remove PersonId = 114420.  This account contains a account creation date of 1900-01-01.
			   ...
			   CREATE TABLE #CoachedAthleteInfo (CoachId INT, CoachingGroupId INT, TotalTriathletes INT, TotalRunners INT, TotalCyclists INT, TotalOtherSports INT, TotalUserType1 INT,  
			                                     TotalUserType4 INT, TotalUserType6 INT, TotalNumberOfLogins INT, NumberOfDaysSinceLastLogon INT, TotalCoachedAthletes INT)
			   INSERT Into #CoachedAthleteInfo
			   EXEC sp_CpapCoachedAthleteInfo  @SourceDbName, @DestinationDbName

			   CREATE TABLE #PlanInfo (CoachId INT, BoughtWorkoutPlan INT, BoughtMealPlan INT, 
			   ...
			   EXEC sp_CpapCoachPlanInfo  @SourceDbName, @DestinationDbName, @WhereCondition, @WhereConditionYear 
	   
			   CREATE TABLE #MealInfo (CoachId INT, PersonId INT, CoachedFromDate DATE, CoachedToDate DATE, PlannedMealCount INT,  PlannedUnLoggedMealCount INT, PlannedLoggedMealCount INT, UnPlannedLoggedMealCount INT,  
			   ...
			   Select * Into #CoachedAthleteMealInfo From 
			   (Select CoachId
			   		  ,Sum(m.PlannedMealCount) As TotalAthletesPlannedMealCount
					  ,Sum(m.PlannedUnLoggedMealCount) As TotalAthletesPlannedUnLoggedMealCount
					  ,Sum(m.PlannedLoggedMealCount) As TotalAthletesPlannedLoggedMealCount
					  ,Sum(m.UnPlannedLoggedMealCount) As TotalAthletesUnPlannedLoggedMealCount
					  ,Sum(m.PlannedMealMonths) As TotalAthletesPlannedMealMonths
					  ,Sum(m.PlannedUnLoggedMealMonths) As TotalAthletesPlannedUnLoggedMealMonths
					  ,Sum(m.PlannedLoggedMealMonths) As TotalAthletesPlannedLoggedMealMonths
					  ,Sum(m.UnPlannedLoggedMealMonths) As TotalAthletesUnPlannedLoggedMealMonths	 
		      From #MealInfo As m 
		      Group By CoachId
		      ) As d;		   
	   
			  CREATE TABLE #RouteInfo (CoachId INT, PersonId INT, CoachedFromDate DATE, CoachedToDate DATE, RouteName VARCHAR(500))
			  INSERT Into #RouteInfo
			  EXEC sp_CpapCoachRouteInfo @SourceDbName, @DestinationDbName
	  
			  Select * Into #CoachedAthleteRouteInfo From 
			  (Select CoachId, Count(*) As TotalAthletesRouteCount
		      From #RouteInfo As r 
		      Group By coachId
		      ) As d;	
   
		      CREATE TABLE #MetricInfo (CoachId INT, PersonId INT, CoachedFromDate DATE, CoachedToDate DATE, WeightInKilogramsMetric INT, SleepQualityMetric INT, SleepHoursMetric INT, FatigueMetric INT, SorenessMetric INT, 
			  ...
			  -- Use DateCreated Rather then TimeStamp As in AthleteInfo
			  EXEC sp_CpapCoachedAthleteMetricInfo @SourceDbName, @DestinationDbName

	  Select * Into #CoachedAthleteMetricInfo From 
			  (Select CoachId
			   		 ,Sum(Case When WeightInKilogramsMetric>0 Then 1 Else 0 End) As TotalAthleteWeightInKilogramsMetric
					 ,Sum(Case When SleepQualityMetric>0 Then 1 Else 0 End) As TotalAthleteWithSleepQualityMetric
					 ,Sum(Case When SleepHoursMetric>0 Then 1 Else 0 End) As TotalAthleteWithSleepHoursMetric
					 ,Sum(Case When FatigueMetric>0 Then 1 Else 0 End) As TotalAthleteWithFatigueMetric
					 ,Sum(Case When SorenessMetric>0 Then 1 Else 0 End) As TotalAthleteWithSorenessMetric
					 ,Sum(Case When StressMetric>0 Then 1 Else 0 End) As TotalAthleteWithStressMetric
					 ,Sum(Case When PulseMetric>0 Then 1 Else 0 End) As TotalAthleteWithPulseMetric
					 ,Sum(Case When SleepElevationInMetersMetric>0 Then 1 Else 0 End) As TotalAthleteWithSleepElevationInMetersMetric
					 ,Sum(Case When OverallFeelingMetric>0 Then 1 Else 0 End) As TotalAthleteWithOverallFeelingMetric
					 ,Sum(Case When PercentFatMetric>0 Then 1 Else 0 End) As TotalAthleteWithPercentFatMetric
		      From #MetricInfo As m 
		      Group By CoachId
		      ) As d;	

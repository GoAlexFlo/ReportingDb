...
Create PROCEDURE [dbo].[sp_TeamSkyEmailCsvWeightMetricTEST]
AS
BEGIN
SET NOCOUNT ON;
...
(
    ...
    (
		...
		UNION ALL
		SELECT i.FullName,  
			   CONVERT(VARCHAR(10), p.[Timestamp], 103) AS [WorkoutDayDD/MM/YYYY], 
			   CONVERT(VARCHAR(10), p.[Timestamp], 103) AS [WorkoutDayYYYY-MM-DD],
			   ''WEIGHT METRIC'' AS Title,
			   NULL AS StartTime, 
			   NULL AS TotalDurationInMinutes, 
			   NULL AS DistanceInMeters, 
			   NULL AS TotalElevationGain, 
			   NULL AS TotalTSS, 
			   NULL AS PowerMM5Seconds, 
			   NULL AS PowerMM10Seconds, 
			   NULL AS PowerMM12Seconds, 
			   NULL AS PowerMM20Seconds, 
			   NULL AS PowerMM30Seconds,
			   NULL AS PowerMM1Minute, 
			   NULL AS PowerMM2Minutes, 
			   NULL AS PowerMM5Minutes, 
			   NULL AS PowerMM6Minutes, 
			   NULL AS PowerMM10Minutes, 
			   NULL AS PowerMM12Minutes, 
			   NULL AS PowerMM20Minutes, 
			   NULL AS PowerMM30Minutes, 
			   NULL AS PowerMM1Hour, 
			   NULL AS PowerMM90Minutes, 
			   NULL AS PowerMM3Hours,  
		       CONVERT(VARCHAR(25), p.WeightInKilograms) AS WeightInKilograms
		FROM Info AS i Inner Join PersonMetric As p On p.personid = i.PersonId 
		WHERE CONVERT(varchar(10), p.[Timestamp], 111) >= ''2013/11/01''  
	)
...
This is only a test. There was a total of 41 weight metric entries since 2013/11/01.  Total number of workouts since 2013/11/01 (4757) plus total number of Weight Metric (41) proof to 4798 rows.   
...
';
    ...
    @recipients=N'ben@peaksware.com',
 -- @recipients=N'white@teamsky.com',
    @copy_recipients =N'AFlores@Peaksware.com',
    @blind_copy_recipients=N'ben@peaksware.com',
    @body= @emailBodyMsg,
    @subject ='WORKOUT DATA FROM 11-01-2013 Weight Metric Mod',
    @reply_to =N'ben@peaksware.com',
